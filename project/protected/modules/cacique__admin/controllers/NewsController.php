...
class NewsController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if(isset($_POST['Noticias']))
        {
            ...
            $existUrl = Noticias::model()->findAllByAttributes(array('post__url'=>$model->post__url));
            if(count($existUrl) > 0)
            ...
            if($model->validate(null, false)){
                ...
                if(($_FILES['image']['size'] > 0)){
                    ...
                    $uploadLogo = MyMethods::uploadImage($_FILES['image'], 600, 'news/');
                    ...
                    if(!$uploadLogo['status']){
                        ...
                        MyMethods::resizeImage($server.'news/', $model->imagen, 400, 300);
                    }
                }
    ...
    public function actionUpdate($id)
    {
        ...
        if(isset($_POST['Noticias']))
        {
            ...
            $model->attributes=$_POST['Noticias'];
             $model->post__url = MyMethods::normalizarUrl(strip_tags($model->titulo));
            $existUrl = Noticias::model()->findAllByAttributes(array('post__url'=>$model->post__url), array('condition'=>'t.id != '.$model->id));
            if(count($existUrl) > 0)
                $model->post__url = (count($existUrl) + 1).'_'.$model->post__url;
                ...
                if(($_FILES['image']['size'] > 0)){
                    ...
                    if(!$uploadLogo['status']){
                        ...
                        if($currentImage != ""){
                            ...
                            @unlink($_SERVER['DOCUMENT_ROOT'].Yii::app()->request->baseUrl."/images/news/400x300/".$currentImage);
                        }
                    }
                }
